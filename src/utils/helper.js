...
const getVideoYoutubeDetails = async (url) => {
    ...
    const getTitle = await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${videoId}&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`);
    const getDuration = await axios.get(`https://www.googleapis.com/youtube/v3/videos?id=${videoId}&part=contentDetails&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`);
...
function getWorkoutImageSrc(title) {
...
export { dateFormatter, urlPattern, getVideoYoutubeDetails, convertISO8601DurationToMinutes, getWorkoutImageSrc };